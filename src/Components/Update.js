...
newText2=((text.split(" ").filter( (element)=>{return element.length!==0})).length);
...
return final;
  ...
  return (
    <div>
      <Alert alert={alert}/>
      <h1 className={`${props.mode}`}>Examine Text</h1>
    <div className='parent'>
    <div className='child'>
      <textarea className={`textarea  ${props.mode}`} value={text} onChange={handleonchange} id='txtarea'></textarea>
     
</div>

<div className={`child mx-5 ${props.mode}`}>
<p>
  {handleonclick_C()}
